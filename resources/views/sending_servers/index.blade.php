...
@section('page_header')
    ...
    <div class="page-title">
        ...
        <h1>
            <span class="text-semibold"><span class="material-symbols-rounded">format_list_bulleted</span> {{ trans('messages.sending_servers') }}</span>
        </h1>
    </div>
...
@section('content')
    ...
    >
        <div class="d-flex top-list-controls top-sticky-content">
                    ...
                    <div class="filter-box">
                        ...
                        <div class="dropdown list_actions" style="display: none">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-item" link-confirm="{{ trans('messages.enable_sending_servers_confirm') }}" href="{{ action('SendingServerController@enable') }}"><span class="material-symbols-rounded">play_arrow</span> {{ trans('messages.enable') }}</a></li>
                                <li><a class="dropdown-item" link-confirm="{{ trans('messages.disable_sending_servers_confirm') }}" href="{{ action('SendingServerController@disable') }}"><span class="material-symbols-rounded">hide_source</span> {{ trans('messages.disable') }}</a></li>
                                <li><a class="dropdown-item" link-confirm="{{ trans('messages.delete_sending_servers_confirm') }}" href="{{ action('SendingServerController@delete') }}"><span class="material-symbols-rounded">delete_outline</span> {{ trans('messages.delete') }}</a></li>
                            </ul>
                        </div>
            ...
            @if (Auth::user()->customer->can('create', Acelle\Model\SendingServer::class))
                <div class="text-end">
                    <a href="{{ action('SendingServerController@select') }}" role="button" class="btn btn-secondary add-btn">
                        <i class="fa-solid fa-plus"></i>
                        <p>{{ trans('messages.create_sending_server') }}</p>
                    </a>
                </div>
            @endif
        </div>
    ...
    <script>
        var SendingServersIndex = {
            getList: function() {
                return makeList({
                    url: '{{ action('SendingServerController@listing') }}',
