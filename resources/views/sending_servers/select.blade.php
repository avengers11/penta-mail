...
@section('page_header')
    ...
    <div class="page-title">
        ...
        <h1>
            <span class="text-semibold"><span class="material-symbols-rounded">add</span> {{ trans('messages.select_sending_servers_type') }}</span>
        </h1>
    </div>
        ...
        <div class="col-md-12">
            <ul class="modern-listing big-icon no-top-border-list mt-0">
                ...
                @foreach (Auth::user()->customer->getSendingServertypes() as $key => $type)
                    ...
                    <li>
                        <a href="{{ action('SendingServerController@create', ["type" => $key]) }}" class="btn btn-secondary add-btn">
                            <i class="fa-solid fa-plus"></i>
                            <p>{{ trans('messages.choose') }}</p>
                        </a>
                        ...
                        <h4><a href="{{ action('SendingServerController@create', ["type" => $key]) }}">{{ trans('messages.sending_server.type.' . $key) }}</a></h4>
                        <p>
                            {{ trans('messages.sending_server.intro.' . $key) }}
                        </p>
                    </li>
                @endforeach
            </ul>
            <div class="">
                <a href="{{ action('SendingServerController@index') }}" role="button" class="btn btn-secondary delete-btn" style="width: 80px">
