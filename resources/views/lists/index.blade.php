...
@section('page_header')
	...
	<div class="page-title">
		...
		<h1>
			<span class="text-semibold"><span class="material-symbols-rounded">format_list_bulleted</span> {{ trans('messages.my_lists') }}</span>
		</h1>
	</div>
        ...
        <div class="d-flex top-list-controls top-sticky-content">
            ...
            <div class="text-end">
                @if (Auth::user()->customer->can('create', new Acelle\Model\MailList()))
                    <a href="{{ action("MailListController@create") }}" role="button" class="btn btn-secondary add-btn">
                        <i class="fa-solid fa-plus"></i>
                        <p>{{ trans('messages.create_list') }}</p>
                    </a>
                @endif
            </div>
        </div>
