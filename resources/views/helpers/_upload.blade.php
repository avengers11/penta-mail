    ...
    <div class="sub_section">
        ...
        <div class="media profile-image d-flex">
            <div class="preview me-4 position-relative" id="{{$dragId}}">
                <a href="{{ $srcOrigin ?? $src }}" image-popup="link" class="upload-media-container radius-0 pre-upload-photo">
                    <img preview-for="image" empty-src="{{ AppUrl::asset('images/placeholder.jpg') }}" src="{{ $src }}" class="rounded-circle" alt="">
                </a>
                <span onclick="$('input[name=image]').trigger('click')"
                    style="position:absolute;top:39px"
                    class="edit-photo text-center">
                    <span class="material-symbols-rounded">edit</span></span>
            ...
            <div class="col-md-10 padding-l0">
                ...
                <a href="#upload" onclick="$('input[name=image]').trigger('click')" class="btn black-btn" style="width: 120px">
                    <i class="fa-solid fa-upload"></i>
                    <p>{{ trans('messages.upload_photo') }}</p>
                </a>
            </div>
        </div>
    </div>
...
<script>
	$(function() {
		...
		$("input.previewable").on('change', function() {
			...
			previewImageBrowse(this, img, function() {
                // 
                img.closest('a').attr("href", img.attr("src"));
            });
		});
