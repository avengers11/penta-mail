...
<h2 class="text-semibold text-primary">{{ trans('messages.click_log') }}</h2>
...
<button type="button" class="btn btn-light sort-direction" data-popup="tooltip" title="{{ trans('messages.change_sort_direction') }}" role="button" class="btn btn-xs">
            ...
            <div class="text-end">
                <a href="javascript:goToDownLoad('click_logs')" role="button" class="btn btn-secondary save-btn">
                    <i class="fa-solid fa-circle-down"></i>
                    <p>{{ trans('messages.click_log.download') }}</p>
                </a>
            </div>
        </div>
    ...
    <script>
        var CampaignLogIndex = {
            getList: function() {
                return makeList({
                    url: '{{ action('CampaignController@clickLogListing', $campaign->uid) }}',
