            ...
            <tr class="position-relative">
                ...
                <td width="15%" class="text-center">
                    <span class="text-muted2 list-status pull-left" title='{{ $campaign->isError() ? $campaign->extractErrorMessage() : '' }}' data-popup='tooltip'>
                        <span title="{{ $campaign->isSending() ? $delayNote : '' }}" class="label label-flat bg-{{ $campaign->status }}">{{ trans('messages.campaign_status_' . $campaign->status) }}</span>
                    </span>
                ...
                <td class="text-end text-nowrap pe-0">
                    <div class="d-flex align-items-center text-nowrap justify-content-end">
                        @if (\Gate::allows('update', $campaign))
                            <a href="{{ action('CampaignController@edit', $campaign->uid) }}" role="button" class="btn btn-secondary btn-icon ms-1 edit-btn">
                                <i class="fa-solid fa-ellipsis"></i>
                                <p>{{ trans('messages.edit') }}</p>
                            </a>
                        @endif
                        @if (\Gate::allows('overview', $campaign))
                            <a href="{{ action('CampaignController@overview', $campaign->uid) }}" data-popup="tooltip" title="{{ trans('messages.overview') }}" role="button" class="btn btn-primary btn-icon ms-1 black-btn">
                                <i class="fa-solid fa-chart-pie"></i>
                                <p>{{ trans('messages.campaign.overview_statistics') }}</p>
                            </a>
                        @endif
                        @if (\Gate::allows('delete', $campaign) || \Gate::allows('pause', $campaign) || \Gate::allows('restart', $campaign))
                            <div class="btn-group ms-1" role="group">
                                <button id="btnGroupDrop1" role="button" class="btn btn-light dropdown-toggle save-btn" data-bs-toggle="dropdown">
                                    <i class="fa-solid fa-ellipsis"></i>
                                    <p>{{ trans('messages.view') }}</p>
                                </button>

                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="btnGroupDrop1">
                                    @if (config('custom.dryrun'))
                                        <li><a class="dropdown-item"
                                            href="{{ action('CampaignController@debug', ["uid" => $campaign->uid]) }}">
